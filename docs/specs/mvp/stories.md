...
- 🔴 **xrun-detection mechanism is undecided** — `XrunCounter` is wired into shared state and the GUI reads it, but the callback never increments it because cpal-via-nih-plug standalone does not surface `StreamError::InputUnderflow` to us (see [`src/audio/plugin.rs`](../../../src/audio/plugin.rs) and [`src/gui/editor.rs`](../../../src/gui/editor.rs)). The TR must pick one of: (a) wall-clock budget check inside `process()` comparing elapsed time to buffer duration, (b) fork nih-plug to expose the cpal stream-error event, (c) raw `cpal` runner alongside nih-plug. Decision blocks mvp-03 and ripples into mvp-04/05's "xrun == 0" assertions. Aligns with `dependencies.md` patterns section.
- 🔴 **Loopback capture primitive for latency (mvp-02)** — the existing 440 Hz test-signal toggle in [`src/audio/plugin.rs`](../../../src/audio/plugin.rs) is sine-based, not impulse-based, and there is no input-capture buffer + trigger/done state machine. The TR must specify how the GUI starts a measurement, how the callback fills a preallocated capture buffer with the loopback impulse, and how completion is signaled back to the GUI. Per A2 the capture buffer must be preallocated, no allocation on the hot path. Ties into the deferred `triple_buffer` choice in `dependencies.md`.
